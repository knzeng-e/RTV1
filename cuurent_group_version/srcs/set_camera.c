...
void    show_vect(t_vect vect)
{
    printf("\nvect ==> (%f, %f, %f)", vect.vect_x, vect.vect_y, vect.vect_z);
    fflush(stdout);
}
...
void	set_camera(t_ray *ray, t_params *params, int i, int j)
{
	...
	pix_remap_x -= params->eye.to.vect_x;
	pix_remap_y -= params->eye.to.vect_y;
	pix_camera_x = pix_remap_x * tan(radians(params->fov) / 2); // FOV 30 degrees
	pix_camera_y = pix_remap_y * tan(radians(params->fov) / 2);
    //show_vect(params->eye.from);
//	p_camera_space = set_vector(pix_camera_x, pix_camera_y, params->eye.from.vect_z);
	p_camera_space = set_vector(pix_camera_x, pix_camera_y, params->eye.from.vect_z - DISTANCE);
